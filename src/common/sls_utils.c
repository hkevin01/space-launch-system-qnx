...
const char* sls_subsystem_type_to_string(subsystem_type_t type) {
    switch (type) {
        case SUBSYS_FLIGHT_CONTROL: return "Flight Control";
        case SUBSYS_ENGINE_CONTROL: return "Engine Control";
        case SUBSYS_TELEMETRY: return "Telemetry";
        case SUBSYS_ENVIRONMENTAL: return "Environmental";
        case SUBSYS_GROUND_SUPPORT: return "Ground Support";
        case SUBSYS_NAVIGATION: return "Navigation";
        case SUBSYS_POWER: return "Power";
        case SUBSYS_THERMAL: return "Thermal";
...
const char* sls_mission_phase_to_string(mission_phase_t phase) {
    switch (phase) {
        case PHASE_PRELAUNCH: return "Pre-launch";
        case PHASE_IGNITION: return "Ignition";
        case PHASE_LIFTOFF: return "Liftoff";
        case PHASE_ASCENT: return "Ascent";
        case PHASE_STAGE_SEPARATION: return "Stage Separation";
        case PHASE_ORBIT_INSERTION: return "Orbit Insertion";
        case PHASE_MISSION_COMPLETE: return "Mission Complete";
...
bool sls_validate_telemetry_point(const telemetry_point_t* point) {
    ...
    if (age > 10.0 || age < -1.0) { // More than 10 seconds old or 1 second in future
        return false;
    }
...
bool sls_validate_vehicle_state(const vehicle_state_t* state) {
    ...
    if (state->altitude < -500.0 || state->altitude > 1000000.0) { // -500m to 1000km
        return false;
    }
...
void* get_subsystem_thread_func(subsystem_type_t type) {
    switch (type) {
        case SUBSYS_FLIGHT_CONTROL: return flight_control_thread;
        case SUBSYS_ENGINE_CONTROL: return engine_control_thread;
        case SUBSYS_TELEMETRY: return telemetry_thread;
        case SUBSYS_ENVIRONMENTAL: return environmental_thread;
        case SUBSYS_GROUND_SUPPORT: return ground_support_thread;
        case SUBSYS_NAVIGATION: return navigation_thread;
        case SUBSYS_POWER: return power_thread;
        case SUBSYS_THERMAL: return thermal_thread;
        default: return NULL;
    }
}
